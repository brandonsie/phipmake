...
* `plans` specified the four sub-workflows in the Larman Lab's `phipmake` pipeline that can be toggled on/off. Targets are delimited by hyphens "-". For example, to run only Enrichment and Polyclonal parts of the pipeline, excluding Counts and AVARDA, set `plans="Enrichment-Polyclonal"`.   
